...
var lodash = require('lodash'),
    ...
    addCellBackground = function (self, column, row, pos, index, isHeader) {
        ...
        self.pdf
            ...
            .fill(isHeader ? column.headerFill : column.fill);
    ...
    addCellBorder = function (self, column, row, pos, isHeader) {
        ...
        var border = isHeader ? column.headerBorder : column.border,
            ...
            doStroke = function () {
                ...
                self.pdf.lineCap('square').strokeColor(column.borderColor || 'black').opacity(opacity || 1).stroke().restore();
            };
    ...
    addCell = function (self, column, row, pos, isHeader) {
        var width = column.width,
            ...
            y = pos.y,
            x = pos.x,
            color = isHeader ? column.headerColor : column.color;
		...
		if (!Array.isArray(data)){
			self.pdf
			.fill(color || 'black')
			.text(data, x, y, lodash.assign({}, column, {
				height: row._renderedContent.height,
				width: width
			}));
		}
		else {
			self.pdf.fill('black').text('', x, y, lodash.assign({}, column, {
				height: row._renderedContent.height,
				width: width,
				continued: true
			}));
			data.forEach(item=>{
				self.pdf.fill(color || 'black').text(item.text, item.params);
			})
			self.pdf.fill('black').text('', {
				continued: false
			});
		}
