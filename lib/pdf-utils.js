...
class PDFUtils {
			...
			p: (doc, item)=>{
				...
				if (item.isHtml){
					...
					item.value.forEach((node, index)=>{
						...
						if ((parentTagName ==='ul' || parentTagName ==='ol') && node.tagName ==='li'){	
							const liIdent=15;
							const firstIdent=7;
	...
	generatePdf(pdfFileName, blocks, setFilePages, noImageMode){
		...
		blocks.forEach((item, i)=>{
			...
			if ((item.type=='lessonPlanHeader') && blocks[i+1] && blocks[i+1].type==='p' && ((blocks[i+1].blockHeight && doc.y+blocks[i+1].blockHeight>740) || (doc.y+item.firstParagraphHeight > 740 && item.sectionHeight < 740))){
				doc.addPage();
			}	
