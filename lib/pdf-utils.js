				...
				if (item.isHtml){
					...
					if (bulletParams){
						...
						if (!bulletParams.isNumber){
							...
							.text((params.index+1)+'.', leftTextIdent+ident-14+(params.listsIdent || 0), doc.y, {
								continued: false,
								align:'left',
							});
							doc.y=oldY;
						...
						if (parentTagName ==='ul' && node.tagName ==='li' && !params.processListsAsBlocks){
							...
							if (node.rawText!=='\n'){
								...
								this.drawActions.p(doc, {
									...
									isHtml: true,
									parentEl: node,
									bulletParams: {style: bulletStyles[ulLevel], isNumber: parentTagName==='ol'},
									params: {
	...
	generatePdf(pdfFileName, blocks, setFilePages, noImageMode){
		...
		blocks.forEach((item, i)=>{
			...
			if ((item.type=='h1' && !item.startOnNewPage) || item.type=='h2' || item.type=='h3' || item.type=='h4'/* || item.type=='lessonPlanHeader'*/){
				...
				if (item.value==="California's Environmental Principles and Concepts"){
				...
				if (item.value==="Science in Action"){
			...
			if (item.type=='p' && _.isArray(item.value)){
				...
				if (text.indexOf('items that students are encouraged to bring in from home')>=0){
