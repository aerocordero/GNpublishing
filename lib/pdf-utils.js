		...
		this.drawActions={
			...
			image:(doc, item)=>{
				...
				if (item.value.indexOf('.gif')<0 && item.value.indexOf('.svg')<0){
					...
					if (item.csvContentProcessing){
						svg=item.csvContentProcessing(svg, doc.x, doc.y);
					}
					doc.addSVG(svg, item.x || doc.x, doc.y, {width: item.width || this.textWidth, height: item.height || undefined});
				}
