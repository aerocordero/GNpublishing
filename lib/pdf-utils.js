			...
			p: (doc, item)=>{
				...
				if (item.isHtml){
					...
					item.value.forEach((node, index)=>{
						...
						const isLastNode=item.value.length===index+1;
						if (parentTagName ==='table'){	
							const tableData=parseHTMLTableElem(item.parentEl, item.value.find(el=>el.tagName==='thead'));
							console.log('Table Item',item, tableData);
							if (tableData.rows?.length){
								...
								tableData.rows.forEach((row, rIndex)=>{
									row.forEach((cell, cIndex)=>{
										...
										if (cell.attributes.colspan){
											for (let i=0; i<parseInt(cell.attributes.colspan)-1; i++){
												row.splice(cIndex+1, 0, {
													text: '',
													attributes: {},
												});
											}
										...
										if (cell.attributes.rowspan){
											for (let i=0; i<parseInt(cell.attributes.rowspan)-1; i++){
												tableData.rows[rIndex+i+1].splice(cIndex, 0, {
													text: '',
													attributes: {},
												});
											}
								...
								const cols=(tableData.headers.length? tableData.headers : tableData.rows[0]).map((cell, index)=>{
									return {
										id:cell.text || index,
										header: cell.text,
										minWidth: doc.widthOfString(cell.text, {})+5,
										attributes: cell.attributes
									}
								});
								let avgTableWidth=width;
								let avgCellWidth=width/cols.length;
								let avgColsNumber=0;
								cols.forEach(col=>{
									if (col.minWidth>avgCellWidth){
										avgTableWidth-=col.minWidth;
								...
								cols.forEach(col=>{
									if (!col.width){
										console.log(avgTableWidth,avgColsNumber,avgCellWidth);
										col.width=avgTableWidth/avgColsNumber;
									}
								});

								const hideHeaders=!tableData.headers.length;
								const rows=(hideHeaders ? tableData.rows : tableData.rows.slice(1));
								console.log(cols, JSON.stringify(rows, null, 4));
								
								this.drawActions.table(doc, {
									hideHeaders,
									columns: cols,
									data: rows.map((row)=>{
										const obj={}
										row.forEach((col,index)=>{
											const id=cols[index]?.id || index;
											obj[id]=col.text;
										});
										return obj;
									})
						...
						else if ((parentTagName ==='ul' || parentTagName ==='ol') && node.tagName ==='li'){	
							const liIdent=15;
							const firstIdent=7;
							...
							const processNode=(node, styles)=>{
								...
								item.startPageNum=this.pageNum;
								if (item.parentItem){
									item.parentItem.startPageNum=this.pageNum;
								}
		...
		blocks.forEach((item, i)=>{
			...
			if ((item.type=='h1' && !item.startOnNewPage) || item.type=='h2' || item.type=='h3' || item.type=='h4'){
				...
				while(blocks[j] && !blocks[j].isTitle && (
					...
					if (blocks[j].type==='p'){
						if (blocks[j].hasImages || item.type=='lessonPlanHeader'){
							item.sectionHeight+=blocks[j].blockHeight
						}
						else {
				...
				if (item.value[0]?.rawText==="Have students work in pairs to read about and do research on artificial selection in animals and plants."){
			...
			if (item.type=='p' && blocks[i+1] && blocks[i+1].isTable && doc.y>650){
				doc.addPage();
			}
			...
			if ((item.type=='lessonPlanHeader') && blocks[i+1] && blocks[i+1].type==='p' && ((blocks[i+1].blockHeight && doc.y+blocks[i+1].blockHeight>740) || (doc.y+item.firstParagraphHeight > 740 && item.sectionHeight < 740) || item.startPageNum!==blocks[i+1].startPageNum)){
				doc.addPage();
			}	
