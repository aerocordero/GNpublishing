...
class PDFUtils {
	...
	constructor (colors, fonts, textIdents){
		...
		this.drawActions={
					...
					item.value.forEach((node, index)=>{
						...
						if (parentTagName ==='ul' && node.tagName ==='li' && !params.processListsAsBlocks){
							...
							doc.fillColor('black')
								...
								.list(lists, leftTextIdent+(params.listsIdent || 0), doc.y, {
									bulletIndent: 50,
									//textIndent: 20,
									bulletRadius:3,
								});	
							...
							if (node.rawText!=='\n'){
								...
								.lineGap(0.3)
								.fontSize(5)
								.text(' ', leftTextIdent, doc.y,{
									width,
									continued: false
								});
								doc.x=leftTextIdent;
								doc.circle(leftTextIdent+3, doc.y+7, 3)	
								   .fill("black");
			...
			introductions: (doc, {value, imgInfo, data, color, titleFont, fontSize, leftIdent}) =>{
				value.forEach(item=>{
			...
			lessonFiles: (doc, {value, file, contentsObj, leftIdent, width, bottomBoxY})=>{
				if (!bottomBoxY){
					bottomBoxY=730;
				}
				let contentsPushed;
				value.forEach(image=>{
					...
					doc
					  ...
					  .moveTo(0, bottomBoxY)
					  //.lineTo(50, 40)
					  .lineTo(650, bottomBoxY)
					  .lineTo(650, bottomBoxY+79)
					  .lineTo(0, bottomBoxY+79)
					  .fill('white');
