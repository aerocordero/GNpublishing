		...
		this.drawActions={
					...
					item.value.forEach((node, index)=>{
						...
						if (parentTagName ==='ul' && node.tagName ==='li' && !params.processListsAsBlocks){
							...
							if (node.rawText!=='\n'){
								...
								if (liHeight+doc.y > 750 && liHeight+doc.y < 770 && liHeight>13 && level===0){
			...
			images:(doc, item)=>{
				...
				item.value.forEach((image, index)=>{
					if (doc.y+image.height>760 && image.x<textIdents.left+40){
						addHighLights();
						doc.addPage();
						y=doc.y;
						maxY=y;
			...
			line: (doc, {leftIdent, width, showOnTopOfThePage}) => {
				if ((doc.y>85 || showOnTopOfThePage) && doc.y<750){
					doc.moveDown(0.4);
					doc.lineWidth(0.7)
						.strokeColor('#999')
			...
			pptSlide: (doc, {value, imgInfo, file, highlight, hideLabels}) =>{
...
//2.17
		blocks.forEach((item, i)=>{
			...
			if (item.type=='p' && blocks[i+1] && (blocks[i+1].type==='image') && !blocks[i+1].dontAttachParagraphToImage && (doc.y+blocks[i+1].height)>740){
			...
			if (item.type=='image' && (doc.y+item.height)>760){
				if (!item.fitToPage){
					doc.addPage();
				}
				else {
					const maxImgHeight=740-doc.y-50;
					if (item.marginTop){
						item.marginTop=0.4;
					}
			...
			if ((item.type=='h1' || item.type=='h2') && blocks[i+1] && blocks[i+1].type==='lessonPlanHeader' && blocks[i+2] && blocks[i+2].blockHeight && doc.y+blocks[i+2].blockHeight>680){
				doc.addPage();
			}		
			...
			if (item.type=='p' && _.isArray(item.value)){
				const textHeight=doc.fontSize(this.defaultFontSize).heightOfString((item.value || []).map(n=>n.text).join(''), {
					width: this.textWidth,
				});		
