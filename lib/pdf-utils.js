...
class PDFUtils {
	...
	constructor (colors, fonts, textIdents){
		...
		this.drawActions={
					...
					item.value.forEach((node, index)=>{
						...
						if (parentTagName ==='ul' && node.tagName ==='li' && !params.processListsAsBlocks){
							...
							if (node.rawText!=='\n'){
								...
								if (doc.y > 720 && ulChild){
									doc.addPage();
								}
								if (doc.y > 720 && parseInt(liHeight)===25){//2 lines
									doc.addPage();
								}
								if (doc.y > 720 && liHeight>26){//1+n lines
									doc.addPage();
								}
						...
						/*
						else if (parentTagName === 'div' && node.nodeType!==3){	
							if (node.rawText!=='\n'){
								console.log(parentTagName, node);
								console.log(parentTagName, node.childNodes.map(n=>n.nodeType));
								//console.log(node.childNodes);
								this.drawActions.p(doc, {
							...
							if (node.rawText!=='\n'){
								console.log(node);
								console.log(node.childNodes.map(n=>n.nodeType));
								this.drawActions.p(doc, {
			...
			table: (doc, item)=>{
				const {columns, data, fontSize, hideHeaders, borderColor, headerColor, leftIdent}=item;
			...
			list: (doc, {value, ident, notMoveDownAfter, html, childUlIdent})=>{
				if (!html){
					html='<ul>';
					value.forEach(str=>{
						html+='<li>'+str+'</li>';
			...
			pptSlide: (doc, item) =>{
				...
				const imgY = doc.y;
				doc.image(value.imagePath, imgX, imgY, {width: width});
				const strokeParams={
					width:1,
					color:'black'
				}
			...
			lessonFiles: (doc, {value, file, contentsObj, leftIdent, width, bottomBoxY, headerParams, rightBoxX, firstPageMove, leftBoxWidth})=>{
				...
				if (this.setFilePages){
					file.pageNum=firstPageNum-this.startPagingPage;
	...
	generatePdf(pdfFileName, blocks, setFilePages){
		...
		const pTextHeight=(item, addLineBreaks)=>{
			if (!item.textHeight){
				item.textHeight=0;
			}
			let text=(item.value || []).map((n, index)=>(['p', 'li'].indexOf(n.tagName)>=0 && index > 0 && addLineBreaks ? '\n' : '')+(n.convertedText || n.text)).join('');
			const textHeight=doc.fontSize(this.defaultFontSize).heightOfString(text, {
				fontSize: this.defaultFontSize,
				width: this.textWidth,
			});	
		...
		blocks.forEach((item, i)=>{
			...
			if ((item.type=='h1' || item.type=='h2') && blocks[i+1] && ['p', 'h2', 'h3'].indexOf(blocks[i+1].type)>=0 && item.startPageNum >0 && blocks[i+1].startPageNum && blocks[i+1].startPageNum === item.startPageNum+1){
				doc.addPage();
			}
			if ((item.type=='h1' && !item.startOnNewPage) || item.type=='h2' || item.type=='h3'){
				...
				while(blocks[j] && (
					(blocks[j].type==='p' && blocks[j].isHtml)
					...
					if (blocks[j].type==='p'){
						item.sectionHeight+=pTextHeight(blocks[j], true);
						if (j>i+1){
				...
				else if (doc.y+item.sectionHeight>730 && item.sectionHeight<400 && doc.y<700 && !item.startOnRightSide){
					item.movingToNextPage=true;
					doc.addPage();
				}
				...
				if (item.value==='COMMON CORE - ELA/Literacy'){
					//console.log('Section', item.value, item.sectionHeight, item.nextSectionElType, doc.y+item.sectionHeight);
				}
			...
			if (item.type=='lessonPlanHeader'){
				...
				let j=i+1;
				let pCounter=0;
				while(blocks[j] && ['p','image','images', 'pptSlide'].indexOf(blocks[j].type)>=0){
					if (blocks[j].type==='p'){
					...
					console.log(blocks[j].type, blocks[j].height || blocks[j].blockHeight);
				...
				if (doc.y+item.sectionHeight>730 && item.sectionHeight<400 && !item.hasImages && item.planIndex>0){
					item.movingToNextPage=true;
					doc.addPage();
				}
				if (item.firstParagraphHeight>200 && doc.y+item.firstParagraphHeight > 730 && item.firstParagraphHeight<600 && doc.y>400){
					item.movingToNextPage=true;
					doc.addPage();
				}
			...
			if ((item.type=='h1' || item.type=='h2' || item.type=='h3') && blocks[i+1] && blocks[i+1].type==='table' && doc.y>590){
				doc.addPage();
			}
