					...
					item.value.forEach((node, index)=>{
						...
						if (parentTagName ==='ul' && node.tagName ==='li' && !params.processListsAsBlocks){
							...
							if (node.rawText!=='\n'){
								...
								const bulletCoordinates=[leftTextIdent+ident+3+(params.listsIdent || 0), doc.y+7];
								if (ulLevel===0){
									doc.circle(bulletCoordinates[0], bulletCoordinates[1], 2.5)	
								   		.fill("black");
								}
								if (ulLevel===1){
									doc.circle(bulletCoordinates[0], bulletCoordinates[1], 2.5)	
		...
		blocks.forEach((item, i)=>{
			...
			if (item.type=='p' && _.isArray(item.value) && blocks[i-1].type==='p'){
				...
				if (textHeight+doc.y>750 && textHeight+doc.y < 770 && (!item.parentEl || (item.parentEl && item.parentEl.tagName!=='ul'))){
					//console.log('textHeight', textHeight, doc.y, item);
					doc.addPage();
				}	
			}	
