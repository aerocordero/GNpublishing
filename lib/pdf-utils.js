...
class PDFUtils {
	...
	generatePdf(pdfFileName, blocks, setFilePages, noImageMode){
		...
		const pTextHeight=(item, addLineBreaks)=>{
			...
			let lis=(item.value || []).filter(n=>n.tagName==='li');
		...
		blocks.forEach((item, i)=>{
			...
			if ((item.type=='h1' && !item.startOnNewPage) || item.type=='h2' || item.type=='h3'/* || item.type=='lessonPlanHeader'*/){
				...
				if (item.value==='2. Online Simulation'){
					console.log('Section', item.value, item.sectionHeight, blocks[i+1], blocks[i+2], blocks[i+3], blocks[i+4], pCounter);
				}
			...
			if (item.type=='p' && blocks[i+1] && blocks[i+1].type==='lessonPlanHeader' && blocks[i+1].isTotalTile && item.blockHeight && doc.y+item.blockHeight>640){
				doc.addPage();
			}	
