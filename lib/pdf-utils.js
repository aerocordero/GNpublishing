...
class PDFUtils {
	...
	constructor (colors, fonts, textIdents){
		...
		this.drawActions={
				...
				if (item.isHtml){
					...
					if (parentTagName === 'tips-box' && parentClass.indexOf('tips-box') >=0) {
						...
						const boxH=doc
							...
							//continued: false
						})+(ident*2)+20+3+(item.value.length > 2 ? (item.value.length-2)*2 : 0);
						if (boxH+doc.y>747){
							doc.addPage();
							//doc.y+=ident;
						}
						...
						if (parentTagName ==='ul' && node.tagName ==='li' && !params.processListsAsBlocks){
							...
							if (node.rawText!=='\n'){
								...
								this.drawActions.p(doc, {
									...
									isHtml: true,
									parentEl: node,
									bulletParams: {style: bulletStyles[ulLevel], isNumber: parentTagName==='ol', listStyleType: parentStyles['list-style-type']},
									params: {
			...
			image:(doc, item)=>{
				...
				//console.log(doc.x, doc.y);
				//console.log(item);
				if (item.value.indexOf('.gif')<0 && item.value.indexOf('.svg')<0){
				...
				else if (item.value.indexOf('.svg')>0){
			...
			table: (doc, item)=>{
				let {columns, data, fontSize, hideHeaders, borderColor, headerColor, leftIdent, padding, marginTop}=item;
				if (!padding){
					padding=4;
				}
			...
			lessonFiles: (doc, {value, file, contentsObj, leftIdent, width, bottomBoxY, headerParams, footerParams, rightBoxX, firstPageMove, leftBoxWidth, marginTop})=>{
				if (!bottomBoxY){
					bottomBoxY=730;
				}
				let contentsPushed;
	...
	writeHeader(doc, header, pageNum){
		...
		if (header.type==='outerBox'){
			...
			.fontSize(32)
			.fill(header.textColor || header.color || this.colors.unitTitle || 'black')
			.text(header.chapter.name, this.textIdents.left, (header.titleLeftTopIdent || 25)+30,{
				width: this.textWidth,
				align: 'center',
				//underline: true
			});
			...
			.fontSize(header.titleLeftFontSize || 16)
			.fill(header.textColor || header.color || this.colors.unitTitle || 'black')
			.text(header.titleLeft, this.textIdents.left, (header.titleLeftTopIdent || 25)-(titleLeftHeight > 24 ? 13 : 0));
			...
			const lineEndX=header.lineEndX || 578;
	...
	writeFooter(doc, pageNum, startPagingPage, footerData){
		...
		if (chapterBadge.bageNum>1){
			chapterBadge.top+=(chapterBadge.height+chapterBadge.innerIdent)*chapterBadge.bageNum;
		}
	...
	generatePdf(pdfFileName, blocks, setFilePages, noImageMode){
		...
		blocks.forEach((item, i)=>{
			...
			if (item.type=='p' && _.isArray(item.value)){
				...
				if (item.hasImages && item.blockHeight+doc.y >= 740 ){
					doc.addPage();
				}
				
				if (text.indexOf('As of 2017, the world population was 7.6 billion. It is expected to reach nearly')>=0){
					console.log('textHeight', textHeight, doc.y, textHeight+doc.y, text, doc.page.margins, item, blocks[i+1], blocks[i+2], blocks[i+3]);				
				}
