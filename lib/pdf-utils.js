	...
	generatePdf(pdfFileName, blocks, setFilePages, noImageMode){
		...
		blocks.forEach((item, i)=>{
			...
			if ((item.type=='h1' && !item.startOnNewPage) || item.type=='h2' || item.type=='h3' || item.type=='h4'/* || item.type=='lessonPlanHeader'*/){
				...
				if (item.value==='Story Creation: How Can You Help Lower Carbon'){
					console.log('Section Size', item.value, item.sectionHeight, doc.y, doc.y+item.sectionHeight, blocks[i-1],item, blocks[i+1], blocks[i+2], blocks[i+3], blocks[i+4], pCounter);
				}
				if (blocks[j]){
				...
				else if (doc.y+item.sectionHeight>720 && item.sectionHeight<610 && doc.y<720 && !item.startOnRightSide){
					item.movingToNextPage=true;
					doc.addPage();
				}
