...
class PDFUtils {
	...
	generatePdf(pdfFileName, blocks, setFilePages, noImageMode){
		...
		blocks.forEach((item, i)=>{
			...
			if ((item.type=='lessonPlanHeader') && blocks[i+1] && blocks[i+1].type==='p' && ((blocks[i+1].blockHeight && doc.y+blocks[i+1].blockHeight>740) || (doc.y+item.sectionHeight > 740 && item.sectionHeight < 740))){
				doc.addPage();
			}	
