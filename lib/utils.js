	...
	if (file.type==='pptx'){
		...
		if (fs.existsSync(saveDir+'/extracted/ppt/notesSlides')){
			fs.readdirSync(saveDir+'/extracted/ppt/notesSlides').forEach(fileName=>{
				...
				if (ext==='xml'){
					const slideNum=parseInt(fileName.replace('notesSlide', '').replace('.xml', ''));
					const xml=fs.readFileSync(saveDir+'/extracted/ppt/notesSlides/'+fileName, 'UTF-8');
					const root=parse(xml);
					const body=root.querySelector('p:txBody');
					let str='';
					body.childNodes.forEach(node=>{
						if (node.tagName==='a:p' && node.text && parseInt(node.text)!=node.text){
							...
							node.childNodes.forEach(node=>{
								if (node.text){
									...
									str+='<'+tag+'>'+node.text.replace('    ', ' ')+'</'+tag+'>';
								}
								else {
									const bullet=node.querySelector('a:buChar');
									if (bullet){
										str+=bullet.getAttribute('char')+' ';
									}
								}
							});
							str+='</p>';
...
const parseHTMLIntoBlocks=async(text, params, blocks)=>{
	...
	proceedImages=()=>{
		...
		if (images.length===1){
			let width=images[0].htmlWidth && images[0].htmlWidth < 100 ? images[0].htmlWidth : 465;
			if (params.imgParams && params.imgParams.width > width){
				params.imgParams.width=width;
			}
			else if (params.imgParams && params.imgParams.width) {
