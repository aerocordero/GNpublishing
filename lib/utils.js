...
const gdSevretPath='./.google-credentials.json';
const gdSecret = require('.'+gdSevretPath);
...
const gdResultsSyncFolderId = '1r2nGBOXfyf5RQcTRGE0yKrN2wtLlAt4b';
...
const GDFolderSync=async (folderId, destFolderName, useGdFolder=true)=>{
	await gdriveSync.syncGDrive(folderId, (useGdFolder ? gdriveFolder+'/' :  __dirname+'/../')+destFolderName, {
...
const dbQuery=async (query, params, customPool)=>{
	const myPool=customPool || pool;
	const queryStr=query.join(' ');
	const tmpdest=__dirname+'/../tmp/data/'+hasha(myPool.pool.config.connectionConfig.database+queryStr+JSON.stringify(params)).substr(0, 10)+'.json';
	...
	const res=(await myPool.query(query.join(' '), params))[0];
...
const getGnHostUrl=()=>{
	const poolDb=pool.pool.config.connectionConfig.database;
	return gnSubDomains[poolDb.replace('greenninja_', '')];
}

const apiQuery=async (path, params)=>{
	const poolDb=pool.pool.config.connectionConfig.database;

	let stateSubdomain=params?.state || 'app';
	if (poolDb && gnSubDomains[poolDb.replace('greenninja_', '')]){
		stateSubdomain=gnSubDomains[poolDb.replace('greenninja_', '')];
	}
	const tmpdest=__dirname+'/../tmp/data/'+hasha(stateSubdomain+'_'+path).substr(0, 10)+'.json';
		
	if (fs.existsSync(tmpdest) && !config.alwaysFlushDBCache){
		return require(tmpdest);
	}
	
	const res=(await superagent.get('https://'+stateSubdomain+'.greenninja.org'+path)).body.result;
	fs.writeFileSync(tmpdest, JSON.stringify(res, null, 4));
	return res;
}
...
const initCustomPagesFromDB=async ()=>{
	const fileFields=['Dear-Educator', 'The-Green-Ninja-Approach', 'How-to-Teach-with-Green-Ninja', 'howUnitWorksImage'];
	const record=(await dbQuery([
		'SELECT * FROM `config` t',
		'WHERE t.`name` = "print"'
	], []))[0];
	...
	fileFields.forEach(field=>{
		if (config[field]){
			fileIds.push(config[field]);
		}
	})
	const files=(await dbQuery([
		'SELECT * FROM `file` t',
		'WHERE t.`id` IN ('+fileIds.join(',')+')'
	], []));
	fileFields.forEach(field=>{
		if (config[field]){
			config[field]=files.find(f=>f.id===config[field]);
			if (config[field].s3_filename){
				config[field].path='/getWordDoc?path=/uploads/lessons/'+config[field].s3_filename
			}
		}
	})
	return config;
...
const downloadFile=(url)=>{
	return new Promise((success, error)=>{
		...
		let isGDoc=false;
		url=url.replace('https://greenninja.org/', '/');
		if (url.indexOf('/')===0){
			url='https://'+getGnHostUrl()+'.greenninja.org'+url;
		}
		...
		if (url.indexOf('https://docs.google.com/')===0){
			if (!urlObj.searchParams.get('exportFormat')){
		...
		url=url.replace('/ngssgreenninja.org', '/app.greenninja.us');	
		...
		if (isExternalPdf && urlObj.searchParams.get('id')){
			filename=urlObj.searchParams.get('id')+'.pdf';
		}
...
//		return;
		...
		if (fs.existsSync(dest) && (!isGDoc || (isGDoc && !argv.reloadGdocs))){
		...
		filestream.on('finish', function (res) {
			if (filename==='1V8WxNX6o9xZtNLm3qVlLsroVI07b_leyV6thmdR2koo.pdf'){
				//console.log(filename, {res, agent}, agent.response.headers);
			}
...
const getImgPropheight=(img, width, notRotate)=>{
	return width*(img.rotated && !notRotate ? img.width/img.height : img.height/img.width);
}
const getImgPropWidth=(img, height)=>{
	return height*(!img.rotated ? img.width/img.height : img.height/img.width);
}
...
const convertPptxPdf=async (path, file, useAbsolutePath, lowQuality)=>{
	const filename=file.fileName.replace(/ /igm, '-');
	let saveDir=__dirname+'/../tmp/pptx-export/'+(file.s3_filename?.replace(/ /igm, '-') || filename);
	const pathArr=path.split('/');
	...
	//saveDir=saveDir.replace('.pdf', '');
	console.log('convertPptxPdf', path, saveDir, argv);

	let cmdPrefix='';
	if (file?.google_drive_object && fs.existsSync(saveDir) && file.path?.indexOf('https://docs.google.com/')===0 && argv.reloadGdocs){
		fs.rmSync(saveDir, { recursive: true, force: true });
	}
	if (!fs.existsSync(saveDir)){
		shell.mkdir('-p', saveDir);
	}
	const dirList=fs.readdirSync(saveDir);
		...
		if (fs.existsSync(saveDir+'/extracted/ppt/notesSlides')){
			fs.readdirSync(saveDir+'/extracted/ppt/notesSlides').forEach(fileName=>{
				...
				if (ext==='xml'){
					...
					if (slideNumElement && parseInt(slideNumElement.text)){
					...
					bodies.forEach(body=>{
						body.childNodes.forEach(node=>{
							if (node.tagName==='a:p' && replaceNoteText(node.text) && parseInt(replaceNoteText(node.text))!=node.text){
								console.log('Slide Text', node.text);
								str+='<p>';
					...
					str=str.replace(/\<p\>\<span\>\‹\#\›\<\/span\>\<\/p\>/ig, "");
...
const cleanUpHTML=(html, debug) => {
	...
	if (res.indexOf('Energy Receiver') >0){
	...
	//console.log('html:'+res);
	const decoded=decodeHtml(res)
	if (res.indexOf('Energy Receiver') >0){
		//console.log('html0:'+res.trim());
	}
...
const parseHTMLIntoBlocks=async(text, params, blocks, debug)=>{
	...
	proceedImages=()=>{
		...
		if (images.length===1){
			//console.log('getImageBlockParams', blocks[blocks.length-1], getImageBlockParams(images[0]));
	...
	if (text.tagName==='p' || 1){
		const parseImages=async (node, pushTexts)=>{
			...
			if (node.tagName!=='img'){
				...
				console.log('inline image', imgInfo, params, {pushTexts}, node);
			}
		}
...
const getImgInfoAndRotate=async(path, rotateLandscapes=true)=>{
	return await imageInfo(path, rotateLandscapes);
}
...
async function uploadToGoogleDrive(filename) {
    //await GDFolderSync('1r2nGBOXfyf5RQcTRGE0yKrN2wtLlAt4b', 'test-data', false);

    const service = new GoogleDriveService(gdSevretPath, gdResultsSyncFolderId);
    
    const list=await service.listFiles();
    console.log(list);
    await asyncForEach(list.filter(file=>file.name===filename), async file=>{
        await service.deleteFile(file.id);
    })
    
    const file = fs.createReadStream(path.resolve('./result/'+filename));
    const mimeType = 'application/pdf'

    const response = await service.createFileUpload(filename, file, mimeType)
...
function parseHTMLTableElem( tableEl, expectingHeaderRow ) {
	var columns = Array.from( tableEl.querySelectorAll( 'th' ) ).map( it => it.rawText );
	...
	if ( columns.length == 0 && expectingHeaderRow && rows.length) {
		// get columns for a non-th'd table
		console.log(rows[ 0 ]);
		columns = Array.from( rows[ 0 ].childNodes ).map( it => it.rawText )
