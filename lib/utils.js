...
const cleanUpHTML=(html, debug) => {
	...
	res=document.body.innerHTML
		...
		.replace(/\<\/strong\> \<strong\>/ig, " </strong\><strong>")
...
const parseHTMLIntoBlocks=async(text, params, blocks, debug)=>{
	...
	proceedImages=()=>{
		...
		if (images.length===1){
			...
			if (height>maxHeight){
				width=getImgPropWidth(images[0].imgInfo, maxHeight);
				params.imgParams.width=width;
			}
			if (params.imgParams){
				params.imgParams.width=width;
			...
			console.log('htmlWidth', images[0].htmlWidth, params.imgParams, width, height, getImgPropheight(images[0].imgInfo, width))
			blocks.push({
				type: 'image',
				value: images[0].path,
